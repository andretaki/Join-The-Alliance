// AWS Configuration - Fix malformed credentials
const rawAccessKeyId = process.env.AWS_ACCESS_KEY_ID;
export const AWS_ACCESS_KEY_ID = rawAccessKeyId?.startsWith('=') ? rawAccessKeyId.substring(1) : rawAccessKeyId;
...
if (process.env.NODE_ENV === 'production') {
  ...
  if (!MICROSOFT_GRAPH_WEBHOOK_SECRET) console.error("MICROSOFT_GRAPH_WEBHOOK_SECRET is not defined!");
...
// Google API Configuration (for future integrations)
